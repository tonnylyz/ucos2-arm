...
static inline CSL_ArmGicIntrSupport_t CSL_armGicIsIntrSupported(const CSL_ArmGicDistIntrf *gicInst, uint32_t intrNum) {
    CSL_ArmgicDistributorRegs *gicDistInst = (CSL_ArmgicDistributorRegs *) gicInst->distributor_base;
...
void gic_enable_interrupt(Uint32 intrNum) {
    CSL_ArmgicDistributorRegs *gicDistInst = gCpuIntrf.gicDist->distributor_base;
    if ((intrNum / 32U) < 16U) {
        gicDistInst->GICD_ISENABLER[intrNum / 32U] = CSL_ARM_GICD_SET_CLR_MASK(intrNum);
    }
}

void gic_disable_interrupt(Uint32 intrNum) {
    CSL_ArmgicDistributorRegs *gicDistInst = gCpuIntrf.gicDist->distributor_base;
    if ((intrNum / 32U) < 16U) {
...
void gic_init() {
    ...
    distrIntrf.distributor_base = (void *) SOC_INTC_MPU_DISTRIBUTOR_BASE;
    ...
    gicCpuInst = (CSL_ArmgicGiccRegs *) (gCpuIntrf.cpuIntfBasePtr);
    gicDistInst = (CSL_ArmgicDistributorRegs *) (gCpuIntrf.gicDist->distributor_base);

    if (FALSE == gCpuIntrf.gicDist->initStatus) {
        gCpuIntrf.cpuId = 0U;

        /* Set binary point register */
        /* TODO: Need to check this value
        ** gCpuIntrf.gicConfig.nonSecureBinaryPoint = GicNsBinaryPointGet();
        */
        gCpuIntrf.gicDist->nonSecureBinaryPoint = 0x03;
        ...
        gCpuIntrf.gicDist->maxValidIntr = maskedValue;
        ...
        */
        ...
        for (intrNum = 32U; intrNum < gCpuIntrf.gicDist->maxValidIntr; intrNum += 1U) {
        ...
        for (intrNum = 32U; intrNum < gCpuIntrf.gicDist->maxValidIntr; intrNum += 32U) {
            if ((intrNum / 32U) < 16U) {
        ...
        for (intrNum = 8U; intrNum < gCpuIntrf.gicDist->maxValidIntr; intrNum++) {
            /* limitting to the arrary size of 128 to handle upto 512
             * interrupt lines */
            if (intrNum < size) {
        ...
        gCpuIntrf.gicDist->initStatus = (Uint32) TRUE;
    }

    /*  Initialize CPU Interface sub-module */
    if (FALSE == gCpuIntrf.initStatus) {
        ...
        */
        ...
        /* Non-Secure BPR */
        gicCpuInst->GICC_BPR = gCpuIntrf.gicDist->nonSecureBinaryPoint;
       ...
       {
         gCpuIntrf.iarRegAddress = (uintptr_t) &gicCpuInst->GICC_AIAR;
         gCpuIntrf.eoiRegAddress = (uintptr_t) &gicCpuInst->GICC_AEOIR;
       }
       else
       {
         gCpuIntrf.iarRegAddress = (uintptr_t) &gicCpuInst->GICC_IAR;
         gCpuIntrf.eoiRegAddress = (uintptr_t) &gicCpuInst->GICC_EOIR;
       }
#else
        /*
        ...
        */
        ...
        gCpuIntrf.initStatus = (Uint32) TRUE;
    }
    gic_enable_interrupt(70); // Timer2 ID70
    mmio_write32(0x4a002a4c, 0); // crossbar setting: disable firewall error irq
}
